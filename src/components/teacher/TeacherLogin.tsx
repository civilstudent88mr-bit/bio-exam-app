...
export function TeacherLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!normName || !password.trim()) { notify('نام و رمز عبور را وارد کنید', 'warning'); return; }
    ...
    try {
      if (mode === 'register') {
        ...
        const teacher = await loginTeacher(normName, password);
        if (!teacher) {
  ...
  const handleReset = async () => {
    ...
    try {
      await resetTeacherPassword(normName, resetPhone.trim(), resetPass);
      ...
      <div className="w-full max-w-md animate-slide-up">
        <div className="card p-8">
          <div className="flex flex-col items-center mb-6">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-white shadow-lg mb-3">
              <BookOpen size={32} />
            </div>
            <h2 className="text-xl font-bold">ورود به پنل معلم</h2>
            <p className="text-sm text-muted mt-1">سامانه آزمون‌ساز زیست‌شناسی</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input className="input" value={name} onChange={e => setName(e.target.value)} placeholder="نام خود را وارد کنید" disabled={lockSecs > 0} />
            </div>
            <div>
      ...
      <Modal open={showReset} onClose={() => setShowReset(false)} title="بازیابی رمز عبور" size="sm">
        <div className="space-y-4">
          ...
          <Field label="نام کاربری">
            <input className="input" value={resetName} onChange={e => setResetName(e.target.value)} placeholder="نام کاربری" autoFocus />
          </Field>
